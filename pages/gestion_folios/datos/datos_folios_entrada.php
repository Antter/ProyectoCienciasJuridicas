<?php
   $query = $db->prepare("SELECT * FROM ( SELECT folios.NroFolio, folios.PersonaReferente, unidad_academica.NombreUnidadAcademica AS ENTIDAD, 
                         categorias_folios.NombreCategoria, folios.FechaEntrada, folios.TipoFolio FROM folios INNER JOIN unidad_academica ON 
						 ...
						 organizacion.NombreOrganizacion AS ENTIDAD, categorias_folios.NombreCategoria, folios.FechaEntrada ,folios.TipoFolio 
						 FROM folios INNER JOIN organizacion ON folios.Organizacion = organizacion.Id_Organizacion INNER JOIN categorias_folios ON 
						 folios.categoria = categorias_folios.Id_Categoria) T1 WHERE TipoFolio = 0 ORDER BY FechaEntrada DESC");
    $query->execute();
    $rows = $query->fetchAll();
        if($rows){
            $folios = 1;
